...
class _SosRoutePageState extends State<SosRoutePage> {
  ...
  Future<void> _fetchAddressFromCoords(double lat, double lng) async {
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        if (mounted) {
          setState(() {
            _destinationAddress = parts.isNotEmpty ? parts.join(', ') : 'Lokasi ditemukan';
          });
        }
        return;
      }
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        ...
        if (addr != null) {
          ...
          if (mounted) {
            setState(() {
              _destinationAddress = parts.isNotEmpty ? parts.join(', ') : (response.data['display_name'] ?? 'Lokasi ditemukan');
            });
          }
        }
      }
  ...
  Future<void> _initLocationAndRoute() async {
    try {
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
        if (mounted) {
          setState(() {
            ...
            _errorMessageKey = "gpsPermissionDenied";
          });
        }
        return;
      }
      ...
      // If we cannot obtain current GPS (desktop, simulator, or permission issues),
      // allow viewing the SOS target location as a fallback instead of blocking navigation.
      if (mounted) {
        setState(() {
          _isLoadingRoute = false;
          _currentPosition = null; // keep route empty but allow map to center on target
        });
        // Ensure the map centers on the target coordinates even without current position
        try {
          _mapController.move(LatLng(_targetLat, _targetLng), _currentZoom);
        } catch (_) {}
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (_errorMessageKey == "gpsPermissionDenied") displayError = '${l10n.gpsFailed} — izinkan akses lokasi untuk melihat rute.';
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_errorMessageKey == "gpsPermissionDenied") ...[
