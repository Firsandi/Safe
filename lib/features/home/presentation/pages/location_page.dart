...
class _LocationPageState extends State<LocationPage> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Gagal memuat kontak: ${e.toString().split('\n').first}'),
      ...
      try {
        final sosRes = await dio.get('/api/sos/history/received', options: Options(receiveTimeout: const Duration(seconds: 30)));
        ...
        debugPrint('Error fetching received SOS: $e');
        // SOS fetch error tidak perlu ditampilkan, hanya track
      }
